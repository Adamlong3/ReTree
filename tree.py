...
TREE_STAGE_ORDER = ("draft", "tree_build", "tree_compile", "verify", "commit")
TREE_BUILD_STAGE_ORDER = ("tree_build_copy", "tree_build_heap", "tree_build_visibility")
...
"""
    try:
        module = load_inline(
            name="retree_compact_tail_ext_v1",
            cpp_sources=[cpp_source],
            functions=["compact_tail_inplace"],
            extra_cflags=["-O3"],
            verbose=False,
        )
        logger.info("Loaded inline C++ tail cache compaction extension for tree decoding.")
        return module
    except Exception as exc:
        logger.warning(
